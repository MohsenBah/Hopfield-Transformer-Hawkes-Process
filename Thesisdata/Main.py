...
def train_epoch(model, training_data, optimizer, pred_loss_func, opt):
    ...
    for batch in tqdm(training_data, mininterval=10,
                      desc='  - (Training total)   ', leave=False):
        """ prepare data """
        event_time, time_gap, event_type , xy= map(lambda x: x.to(opt.device), batch)

        """ forward """
        optimizer.zero_grad()

        enc_out, prediction, attn , spatial_input_embedded2= model(event_type, event_time, xy)
    ...
    #torch.save(spatial_input_embedded2, '/content/drive/MyDrive/Thesisdata/encout.train.pt')
    #torch.save(attn, '/content/drive/MyDrive/Thesisdata/attn.train.pt')
    #torch.save(event_type, '/content/drive/MyDrive/Thesisdata/event_type.pt')
    #torch.save(prediction[1], '/content/drive/MyDrive/Thesisdata/event_time_pred1.pt')
    #torch.save(torch.max(prediction[0], dim=-1)[1], '/content/drive/MyDrive/Thesisdata/event_type_ptrain.pt')
    #torch.save(xy, '/content/drive/MyDrive/Thesisdata/xy_pred1.pt')
...
def eval_epoch(model, validation_data, pred_loss_func, opt):
    ...
    total_num_pred = 0  # number of predictions
    with torch.no_grad():
        for batch in tqdm(validation_data, mininterval=10,
            ...
            total_num_event += event_type.ne(Constants.PAD).sum().item()
            total_num_pred += event_type.ne(Constants.PAD).sum().item() - event_time.shape[0]
    #torch.save(attn, '/content/drive/MyDrive/Thesisdata/attn.test.pt')
    #torch.save(prediction[1], '/content/drive/MyDrive/Thesisdata/event_time_pred2.pt')
    #torch.save(torch.max(prediction[0], dim=-1)[1], '/content/drive/MyDrive/Thesisdata/event_type_ptest.pt')
    #torch.save(xy, '/content/drive/MyDrive/Thesisdata/xy_pred2.pt')
    rmse = np.sqrt(total_time_se / total_num_pred)
    return total_event_ll / total_num_event, total_event_rate / total_num_pred, rmse
