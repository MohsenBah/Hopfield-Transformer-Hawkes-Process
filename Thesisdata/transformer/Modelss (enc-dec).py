...
class STAR(torch.nn.Module):
    ...
    def forward(self, event_type, event_time, xy,non_pad_mask):
        ...
        temporal_input_embedded_last = temporal_input_embedded[:,-430:,:]
        temporal_input_embedded = temporal_input_embedded[:,:-430,:]
        fusion_feat = torch.cat((temporal_input_embedded_last, spatial_input_embedded[:,-430:,:]), dim=2)
...
class Hopfield(torch.nn.Module):
    def __init__(
        ...
        self.layer_stackD = nn.ModuleList([
    ...
    def forward(self, event_time_pred, event_type_pred, xy_pred,event_type, enc_output, attn1,attn2,attn3,attn4,attn5,attn6,attn7,attn8,enc1,enc2,enc3,enc4,enc5,enc6,enc7,enc8,non_pad_mask,non_pad_mask2):
        ...
        tem_enc = self.temporal_enc(event_time_pred, non_pad_mask)
        temporal_input_embedded = self.event_emb(event_type_pred)
        ...
        spatial_encc = spatial_encc[:,-344:,:]
        spatial_input_embedded = spatial_input_embedded[:,-344:,:]
        for enc_layer in self.layer_stack:
            spatial_input_embedded += spatial_encc
            spatial_input_embedded, xattn = enc_layer(
                spatial_input_embedded,
                non_pad_mask=non_pad_mask[:,-344:,:])
...
class Transformer(nn.Module):
    ...
    def forward(self, event_type, event_time, xy,event_time_pred, event_type_pred, xy_pred,attn1,attn2,attn3,attn4,attn5,attn6,attn7,attn8,enc1,enc2,enc3,enc4,enc5,enc6,enc7,enc8):
        ...
        dec_output = self.decoder(event_time_pred, event_type_pred, xy_pred,event_type,enc_output, attn1,attn2,attn3,attn4,attn5,attn6,attn7,attn8,enc1,enc2,enc3,enc4,enc5,enc6,enc7,enc8, non_pad_mask,non_pad_mask2)
